...
#### ランダム率（%）の入力仕様
- ランダム率（%）は **0以上95以下の数値**を入力してください。
- 空欄の場合は `0`（揺らぎなし）として扱います。
- `abc` や `20abc` のように数値として解釈できない値を入力した場合はエラーに
  なります（「ランダム率（%）には数値を入力してください。」）。
- 0以上95以下の範囲外の数値（負の値や100以上など）を入力した場合もエラーに
  なります（「ランダム率は0以上95以下の数値を入力してください。」）。
- `20.5` のような小数も入力できます。
...
- `seed` を空欄または `0` にした場合も、`0` を固定シードとして扱います。
  ...
  （常に再現可能な結果になります）。
- `seed` には整数であれば負の値も入力できますが、通常は `0` 以上の整数を
  使うことを推奨します。
